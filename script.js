...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        console.log('⚠️ Zappy: Contact form handler not initialized properly');
      } else {
        (async function() {
          try {
            ...
            if (formData.date) {
              fullMessage = 'תאריך מועדף: ' + formData.date + '\n\n' + fullMessage;
            }
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: 'e73261c2-b647-4231-97bd-7bcbed3342c0',
                name: formData.name,
                email: formData.email,
                subject: 'פנייה חדשה מאתר מיקה שמלות כלה',
                message: fullMessage,
            ...
            if (response.ok) {
              ...
              console.log('⚠️ Zappy: Email sent but received non-OK response', result);
            }
          } catch (error) {
      ...
      alert('תודה רבה על פנייתך! ניצור איתך קשר בהקדם.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מיקה שמלות כלה במחירים שפויים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
